...
const createUser = async (payload) => {
    ...
    if (!payload.password) {
        throw new AppError(httpStatus.StatusCodes.BAD_REQUEST, "Password is required to register an account");
    }
...
const updateMyProfile = async (userId, payload) => {
    ...
    const hasRestrictedFields = restrictedFields.some((field) => Object.keys(payload).includes(field));
...
const updateUserByAdmin = async (userId, payload) => {
    ...
    if (payload.email) {
        throw new AppError(httpStatus.StatusCodes.BAD_REQUEST, "Email address cannot be updated");
    }
    if (payload.password) {
        const saltRounds = Number(envVars.BCRYPT_SALT_ROUND) || 10;
