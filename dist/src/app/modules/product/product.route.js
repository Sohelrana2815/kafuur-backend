...
(req, res, next) => {
    ...
    next();
}, validateRequest(ProductValidation.updateProductZodSchema), ProductControllers.updateProduct);
// HARD DELETE
router.delete("/:id", auth(Role.ADMIN), ProductControllers.deleteProductById);
// SOFT DELETE
router.patch("/soft/:id", auth(Role.ADMIN), ProductControllers.softDeleteProductById);
