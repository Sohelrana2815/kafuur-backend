...
const getOrderSummary = catchAsync(async (req, res) => {
    ...
    if (!idsParam || typeof idsParam !== "string") {
        throw new AppError(httpStatus.StatusCodes.BAD_REQUEST, "Cart item IDs are required");
    }
...
//   });
// });
const deleteSingleCartItem = catchAsync(async (req, res) => {
    const { id } = req.params; // Captures the single ID from the URL
    const userId = req.user?.id; // Extracted from auth middleware
    await CartServices.deleteSingleCartItem(id, userId);
    sendResponse(res, {
        statusCode: httpStatus.StatusCodes.OK,
        success: true,
