 ...
 */
router.post("/register", auth(Role.ADMIN), // Uses your exact auth(...) role validation signature logic
validateRequest(AdminAuthValidation.registerAdminZodSchema), AdminAuthControllers.registerAdmin);
/**
 * Public Endpoint: Gateway used by admins to gain session credentials and enter dashboard
 */
router.post("/login", validateRequest(AdminAuthValidation.loginAdminZodSchema), AdminAuthControllers.loginAdmin);
