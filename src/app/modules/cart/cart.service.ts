...
const getCarts = async (userId: string, cartItemIds?: string[]) => {
  // --- THE FIX STARTS HERE ---
...
const getOrderSummary = async (userId: string, cartItemIds: string[]) => {
  ...
  if (cartItems.length === 0) {
    throw new AppError(
      ...
      "At least one cart item ID is required",
    );
  }

  const itemCount = cartItems.reduce(
    (total, item) => total + item.quantity, // product * quantity; example: A -> 2 + B -> 4 = 6 items
    0,
  );
...
const incrementCartItem = async (userId: string, productId: string) => {
...
const decrementCartItem = async (userId: string, productId: string) => {
...
const deleteSingleCartItem = async (id: string, userId: string) => {
